...
public class StreamPractice {
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        List<Integer> filteredNumbers = IntStream.range(0, numbers.size())
                .map(i -> i % 2 == 1 ? numbers.get(i) - 1 : numbers.get(i))
                .filter(n -> n % 2 == 1)
                .boxed()
                .toList();
        if (filteredNumbers.size() == 0) {
            throw new NoSuchElementException("List" + filteredNumbers
                    + "doesn't contain any odd numbers");
        }
        return filteredNumbers.stream().mapToInt(Integer::intValue).sum()
                / (double) filteredNumbers.size();
