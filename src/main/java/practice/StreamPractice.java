...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
        return numbers.stream()
                .map(arrayString -> arrayString.split(","))
                .flatMapToInt(strings -> Arrays.stream(strings).mapToInt(Integer::parseInt))
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return numbers.stream()
                .map(number -> numbers.indexOf(number) % 2 != 0 ? number - 1 : number)
                .filter(num -> num % 2 != 0)
                .mapToDouble(Integer::doubleValue)
                .distinct()
                .average()
                .orElseThrow(() -> new NoSuchElementException("add numbers don't exist"));
    }
    /**
