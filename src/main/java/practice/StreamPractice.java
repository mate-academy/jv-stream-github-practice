...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
        OptionalInt result = OptionalInt.of(numbers.stream()
                .filter(s -> s.contains(","))
                .map(s -> s.split(","))
                .map(Arrays::asList)
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        ...
        OptionalDouble average = IntStream.range(0, numbers.size())
                .map(i -> (i % 2 != 0) ? (i = numbers.get(i) - 1) : (i = numbers.get(i)))
                .filter(i -> i % 2 != 0)
                .average();
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream().filter(p -> p.getAge() >= fromAge && ((p.getSex()
                        == Person.Sex.MAN && p.getAge() <= maleToAge) || (p.getSex()
                        == Person.Sex.WOMAN && p.getAge() <= femaleToAge)))
                .collect(Collectors.toList());
