...
public class StreamPractice {
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream
                ...
                .filter(i -> i % 2 != 0)
                .average()
                .orElseThrow(() -> new NoSuchElementException("No such list " + numbers));
    }
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        Predicate<Person> conditionForPerson = p -> p.getSex().equals(Person.Sex.MAN)
                && p.getAge() >= fromAge
                && p.getAge() <= toAge;
        return peopleList
                .stream()
                .filter(conditionForPerson)
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        Predicate<Person> conditionForPerson = p ->
                p.getAge() >= fromAge
