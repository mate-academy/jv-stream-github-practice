...
public class StreamPractice {
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream.range(ZEROINDEX,numbers.size())
                .mapToDouble(index -> oddIndexNumberSubtractOne(numbers, index))
                .filter(n -> !isEvenNumber((int) n))
                .average()
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream()
                .filter(p -> selectBySexAndAge(p, femaleToAge, maleToAge, fromAge))
                .toList();
    }
    /**
    ...
    private int oddIndexNumberSubtractOne(List<Integer> numbers, int index) {
        return index % 2 != 0 ? numbers.get(index) - 1 : numbers.get(index);
    }

    private boolean selectBySexAndAge(Person person, int femaleToAge,
                                             int maleToAge, int fromAge) {
