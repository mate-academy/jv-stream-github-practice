...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
        OptionalInt value = numbers.stream()
                .map(w -> w.split(","))
                .flatMap(Arrays::stream)
                .mapToInt(Integer::parseInt)
                .filter(n -> n % 2 == 0)
                .min();
        return value.orElseThrow(() -> new RuntimeException("Can't get min value from list: "
                + numbers));
    }
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        OptionalDouble value = IntStream.range(0, numbers.size())
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        List<Person> selected = peopleList.stream()
                .filter(p -> p.getSex() == MAN
    ...
    public List<String> getCatsNames(List<Person> peopleList, int femaleAge) {
        List<String> selected = peopleList.stream()
                .filter(p -> p.getSex() == WOMAN && p.getAge() >= femaleAge)
                .map(Person::getCats)
