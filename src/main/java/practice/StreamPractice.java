...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
       return numbers.stream()
                ...
                .mapToInt(s -> Integer.parseInt(s)).filter(s -> s % 2 == 0)
                .min()
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        return peopleList.stream()
                .filter(s -> s.getSex().name().equals("MAN") && s.getAge() >= fromAge && s.getAge() <= toAge)
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream().filter(s -> s.getAge() >= fromAge
                && (s.getSex().equals(Person.Sex.MAN)
                     && s.getAge() <= maleToAge || s.getSex().equals(Person.Sex.WOMAN) && s.getAge() <= femaleToAge))
                .collect(Collectors.toList());
    }
    ...
    public List<String> getCatsNames(List<Person> peopleList, int femaleAge) {
        return peopleList.stream()
                ...
                .flatMap(e -> e.getCats().stream().map(Cat::getName)).distinct().collect(Collectors.toList());
    }
