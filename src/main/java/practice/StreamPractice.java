...
public class StreamPractice {
     ...
     */
    public List<Person> getWorkablePeople(int fromAge, int femaleMaxAge,
                                          int maxAge, List<Person> peopleList) {
        Predicate<Person> predicate = p -> (p.getSex().equals(Person.Sex.MAN)
                && p.getAge() >= fromAge && p.getAge() <= maxAge)
                || (p.getSex().equals(Person.Sex.WOMAN)
                && p.getAge() >= fromAge && p.getAge() <= femaleMaxAge);
        return peopleList
                .stream()
                .filter(predicate)
                .toList();
