...
public class StreamPractice {
    private final IntPredicate isOddNumber = n -> n % 2 != 0;
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        return peopleList.stream()
                .filter(militaryFit(fromAge, toAge))
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream()
                .filter(checkWorkable(fromAge, maleToAge, femaleToAge))
                .collect(Collectors.toList());
    }
    ...
    private Predicate<Person> militaryFit(int fromAge, int toAge) {
        return p -> p.getAge() >= fromAge
                && p.getAge() <= toAge
                && p.getSex() == Person.Sex.MAN;
    }

    private IntUnaryOperator decreaseOddNumbers(List<Integer> numbers) {
        return n -> n % 2 != 0 ? numbers.get(n) - 1 : numbers.get(n);
    }

    private Predicate<Person> checkWorkable(int fromAge, int maleToAge, int femaleToAge) {
