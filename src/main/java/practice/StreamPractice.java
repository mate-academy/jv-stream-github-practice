...
public class StreamPractice {
    public static Predicate<Person> isManAge_OK(int fromAge, int toAge) {
    ...
    public static Predicate<Person> isCandidateValid(int fromAge, int toAge) {
        return person -> person.getSex() == Person.Sex.MAN
                && person.getAge() >= fromAge
                && toAge >= person.getAge();
    }
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream.range(0, numbers.size())
                ...
                .orElseThrow(() -> new NoSuchElementException(
                        "The list doesn't contains requested elements"));
    }
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        return peopleList.stream()
                .filter(isManAge_OK(fromAge, toAge))
    ...
    public List<String> getCatsNames(List<Person> peopleList, int femaleAge) {
        return peopleList.stream()
                .filter(i -> (i.getAge() >= femaleAge) && i.getSex() == Person.Sex.WOMAN)
