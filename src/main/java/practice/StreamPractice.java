...
public class StreamPractice {
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream
                ...
                .filter(n -> n % 2 == 1)
                .average()
                .orElseThrow(() -> new NoSuchElementException("Can't get average from: "
                        + numbers));
    }
    ...
    public List<Person> selectMenByAge(List<Person> peopleList, int fromAge, int toAge) {
        Predicate<Person> manFromAgeToAge = person -> person.getSex() == Person.Sex.MAN
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        Predicate<Person> peopleFromAgeToAge = person -> {
    ...
    public List<String> getCatsNames(List<Person> peopleList, int femaleAge) {
        Predicate<Person> womenFromFemaleAge = person ->
                person.getSex() == Person.Sex.WOMAN && person.getAge() >= femaleAge;
        return peopleList.stream()
                .filter(womenFromFemaleAge)
                .map(Person::getCats)
                .flatMap(List::stream)
                .map(Cat::getName)
                .collect(Collectors.toList());
