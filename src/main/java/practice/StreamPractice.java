...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
        String messageNoSuchElExc = "Can't get min value from list";
        return numbers.stream()
                .flatMap(n -> Arrays.stream(n.split(",")))
                .map(Integer::parseInt)
                .filter(x -> x % 2 == 0)
                .min(Integer::compare)
                .orElseThrow(() -> new NoSuchElementException(messageNoSuchElExc));
    }
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream()
                .filter(x -> x.getAge() >= fromAge
                        && (x.getSex() == Person.Sex.MAN && x.getAge() <= maleToAge
                        || x.getSex() == Person.Sex.WOMAN && x.getAge() <= femaleToAge))
                .collect(Collectors.toList());
    }
