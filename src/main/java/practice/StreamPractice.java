...
public class StreamPractice {
    private static boolean test(Candidate c) {
        return new CandidateValidator().test(c);
    }
    ...
    public int findMinEvenNumber(List<String> numbers) {
        return numbers.stream().map(s -> Arrays.stream(s.split(",")))
                .flatMap(s -> s).mapToInt(s -> Integer.valueOf(s))
                .filter(i -> i % 2 == 0).min()
                .orElseThrow(
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream.range(0, numbers.size())
                ...
                .filter(i -> i % 2 == 1).mapToDouble(d -> d).average().getAsDouble();
    }
    ...
    public List<People> selectMenByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList.stream().filter(p -> p.getSex() == People.Sex.MAN
    ...
    public List<String> getCatsNames(List<People> peopleList, int femaleAge) {
        return peopleList.stream()
                ...
                .map(p -> p.getCats().stream()).flatMap(c -> c)
                .map(c -> ((Cat)c).getName()).collect(Collectors.toList());
    }
    ...
    public static List<String> validateCandidates(List<Candidate> candidates) {
        return candidates.stream().filter(StreamPractice::test).map(Candidate::getName)
