...
public class StreamPractice {
    ...
    public int findMinEvenNumber(List<String> numbers) {
        return numbers.stream()
                .flatMap(e -> Arrays.stream(e.split(","))) // flatten to stream of strings
                .mapToInt(Integer::parseInt) // convert each string to integer
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge,
        ...
        Predicate<Person> checkWorkablePeople = new Predicate<Person>() {
            @Override
            public boolean test(Person person) {
                if (person.getAge() >= fromAge) {
                    return ((person.getSex().toString().equals("MAN")
                            && person.getAge() <= maleToAge))
                            || (person.getSex().toString().equals("WOMAN")
