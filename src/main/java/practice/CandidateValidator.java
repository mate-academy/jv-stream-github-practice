...
public class CandidateValidator implements Predicate<Candidate> {
    ...
    private static final int UPPER_BOUND_OF_LIVING = 1;
    private static final int BOTTOM_BOUND_OF_LIVING = 0;

    @Override
    public boolean test(Candidate candidate) {
        String[] splitPeriods = candidate.getPeriodsInUkr().split("-");
        int livingUpperBound = Integer.parseInt(splitPeriods[UPPER_BOUND_OF_LIVING]);
        int livingBottomBound = Integer.parseInt(splitPeriods[BOTTOM_BOUND_OF_LIVING]);
