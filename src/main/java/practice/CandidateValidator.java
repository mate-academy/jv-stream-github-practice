...
public class CandidateValidator {
    public static boolean validate(Candidate candidate) {
        return candidate.getAge() >= 35 &&
                ...
                candidate.getPeriodsInUkr().equals("2002-2015");
    }
}
...
//
//
//Given the list of strings where each element contains 1+ numbers: input = {"5,30,100", "0, 22, 7", ...} return a min integer value. One more thing - we're interested in even numbers. If there is no needed data throw RuntimeException with the message "Can't get min value from list: method_input_list".
//
...
//
//        Given a List of People instances (having name, age, and sex fields), for example, Arrays.asList( new People(«Victor», 16, Sex.MAN), new People(«Helen», 42, Sex.WOMEN)), select from the List only men whose age is from fromAge to toAge inclusively. Example: select men who can be recruited to the army (from 18 to 27 years old inclusively).
//
...
//
//        Given a List of People instances (having name, age, sex and List<Cat> cats fields, and each Cat having a name and age), return the names of all cats whose owners are women from femaleAge years old inclusively.
//
