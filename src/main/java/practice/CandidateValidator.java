...
public class CandidateValidator implements Predicate<Candidate> {
    ...
    private static final int INDEX_FOR_DATE_1 = 1;
    private static final int INDEX_FOR_DATE_2 = 0;

    @Override
    public boolean test(Candidate candidate) {
        String[] splittedPeriod = candidate.getPeriodsInUkr().split("-");
        int periodInUkraine = Integer.parseInt(String.valueOf(splittedPeriod[INDEX_FOR_DATE_1]))
                - Integer.parseInt(String.valueOf(splittedPeriod[INDEX_FOR_DATE_2]));
